	...
	for _, rtype := range allTypes {
		...
		if failed > 0 {
			...
			if existingRestartingCondition != nil {
				...
				// dynamic worker allows job to continue even with failed pods
				// (different failure policy options than pytorchjob - this can be represented
				// by the JobSet FailurePolicy).
...
// pytorch controller does not implement this method. it doesn't get called anywhere in this codebase
// so it must be called upstream somewhere.
